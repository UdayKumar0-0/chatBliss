...
io.on('connection', (socket) => {
  socket.on('joinRoom', ({ username, room }) => {
    ...
    socket.emit('message', formatMessage(botName, 'Welcome to the ChatBliss'));
  ...
  socket.on('disconnect', () => {
    ...
    io.to(user?.room).emit(
      'message',
      formatMessage(botName, `${user?.username} has leave from the ChatBliss`)
    );
  });
});
